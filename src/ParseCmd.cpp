...
bool ClientAgent::ParseCmd(const char* data)
{
	...
	if (firstWord == "/SOUNDPLAY" || firstWord == "/CL-AMP") {
		app_info ai;
		...
		thread_id execThread = spawn_thread(ExecPipe, "exec_thread", B_LOW_PRIORITY, execmsg);
	...
	if (firstWord == "/AWAY") {
		...
		tempString = ExpandKeyed(vision_app->GetCommand(CMD_AWAY).String(), "R", expansions);
	...
	if (firstWord == "/CTCP") {
		BString theTarget(GetWord(data, 2)), theAction(RestOfString(data, 3));
	...
	if (firstWord == "/DCC") {
		BString secondWord(GetWord(data, 2)), theNick(GetWord(data, 3)),
			theFile(RestOfString(data, 4));

		if (secondWord.ICompare("SEND") == 0 && theNick != "-9z99") {
	...
	if (firstWord == "/DOP" || firstWord == "/DEOP" || firstWord == "/DEVOICE") {
		...
		if (theNick != "-9z99") {
			...
			dispString += ": ";
			dispString += B_TRANSLATE("Error: Invalid parameters");
			dispString += "\n";
	...
	if (firstWord == "/PEXEC" || firstWord == "/RRUN") // piped exec
	{
		...
		if (*theCmd != "-9z99") {
			...
			thread_id execThread = spawn_thread(ExecPipe, "exec_thread", B_LOW_PRIORITY, msg);
	...
	if (firstWord == "/EXCLUDE") {
		{
			BString second(GetWord(data, 2)), rest(RestOfString(data, 3));
	...
	if (firstWord == "/MODE") {
		BString theMode(RestOfString(data, 3)), theTarget(GetWord(data, 2));
	...
	if (firstWord == "/NOTICE") {
		BString theTarget(GetWord(data, 2)), theMsg(RestOfString(data, 3));
	...
	if (firstWord == "/QUERY" || firstWord == "/Q") {
		BString theNick(GetWord(data, 2)), theMsg(RestOfString(data, 3));
	...
	if (firstWord == "/QUI" || firstWord == "/QUIT") {
		BString theRest(RestOfString(data, 2)), buffer;
	...
	if (firstWord == "/SETBOOL") {
		BString var(GetWord(data, 2)), value(GetWord(data, 3));
		bool newvalue(false), caught(false);
		value.ToLower();
		...
		if (!caught || value == "-9z99") {
			...
			if (returned == B_OK) {
				BString buffer("[x] /setbool: ");
				buffer += B_TRANSLATE("Bool has been set");
				buffer += "\n";
				Display(buffer.String(), C_ERROR);
			} else {
				BString buffer("[x] /setbool: ");
				buffer += B_TRANSLATE("Error setting bool");
				buffer += "\n";
				Display(buffer.String(), C_ERROR);
			}
		}
	...
	if (firstWord == "/T") {
		BString theChan(fId), cmd(RestOfString(data, 2));
	...
	if (firstWord == "/UPTIME") {
		BString parms(GetWord(data, 2)), uptime(DurationString(system_time())), expandedString;
	...
	if (firstWord == "/VERSION" || firstWord == "/TIME") {
		BString theCmd(firstWord.RemoveFirst("/")), theNick(GetWord(data, 2));
		theCmd.ToUpper();
...
int32 ClientAgent::ExecPipe(void* arg)
{
	...
	if (fp == NULL) {
		msg->what = M_DISPLAY;
		BString buffer("[x] ");
		buffer += B_TRANSLATE("/pexec: command failed");
		buffer += "\n";
...
int32 ClientAgent::DNSLookup(void* arg)
{
	...
	BString resolve(lookup), output("[x] ");
