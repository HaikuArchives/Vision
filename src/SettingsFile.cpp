...
SettingsFile::SettingsFile(char const* lname, char const* bname, directory_which d)
{
	...
	if (bname == NULL) {  // no base name, try to figure it out from the signature
		...
		bool founddot = false;
		char* sep;
		while ((sep = strchr(sig, '.'))
			   != NULL) {  // replace each '.' by a '/' in the signature to build a relative path
			*sep = '/';
			founddot = true;
		}
		if (!founddot
			&& ((sep = strchr(sig, '-'))
				!= NULL)) {	 // no '.' was found. replace the first '-' by a '/', if there's a '-'
			*sep = '/';
		}
...
status_t
SettingsFile::_ExtractAttribute(
	BMessage* m, BFile* f, const char* full_name, char* partial_name, attr_info* ai)
{
	...
	if (end == NULL) {	// found a leaf
		if (!m->HasData(partial_name,
				ai->type)) {  // the name does not exist in the message - ignore it
			return B_OK;
		}
		...
		if (f->ReadAttr(full_name, ai->type, 0, buffer, ai->size)
			!= ai->size) {	// cannot read the data
			free(buffer);
			return B_IO_ERROR;
		}
	...
	if (end[1] != ':') {  // found an un-numbered sub-message
		*(end++) = '\0';  // zero-terminate the name, point to the rest of the sub-string
		if (!m->HasMessage(
				partial_name)) {  // archived message does not contain that entry. go away.
			return B_OK;
		}
		BMessage subm;
		...
		*end = '\0';							 // zero-terminate the name
		...
		if (*endptr == ':') {  // this is a numbered message
			if (!m->HasMessage(partial_name,
					r)) {  // archived message does not contain that entry, go away
				return B_OK;
			}
			BMessage subm;
			...
			if (!m->HasData(partial_name, ai->type,
					r)) {  // archived message does not contain this leaf
				return B_OK;
			}
			void* buffer = malloc(ai->size);
			if (buffer == NULL) {
			...
			if (f->ReadAttr(full_name, ai->type, 0, buffer, ai->size)
				!= ai->size) {	// extract the attribute data
				free(buffer);
				return B_IO_ERROR;
			}
			ret = m->ReplaceData(partial_name, ai->type, r, buffer,
				ai->size);	// and replace it in the message
			if (ret != B_OK) {
				free(buffer);
				return ret;
			}
...
SettingsFile::_StoreAttributes(BMessage const* m, BFile* f, const char* basename)
{
	...
	for (int32 i = 0; i < m->CountNames(B_ANY_TYPE); i++) {	 // walk the entries in the message
		...
		if (strchr(namefound, ':')
			!= NULL) {	// do not process anything that contains a colon (considered a magic char)
			break;
		}
		switch (typefound) {
			case B_MESSAGE_TYPE:
			{							// found a sub-message
				if (countfound == 1) {	// single sub-message
					char* lname = (char*)malloc(strlen(basename) + strlen(namefound)
												+ 2);  // allocate space for the base name
					if (lname == NULL) {
						return B_NO_MEMORY;
					}
					sprintf(lname, "%s%s:", basename,
						namefound);	 // create the base name for the sub-message
					BMessage subm;
					...
					ret = _StoreAttributes(&subm, f,
						lname);	 // and process the sub-message with the base name
					if (ret != B_OK) {
						free(lname);
						return ret;
					}
					...
					char* lname = (char*)malloc(strlen(basename) + strlen(namefound)
												+ 11);	// allocate space for the base name
					if (lname == NULL) {
						return B_NO_MEMORY;
					}
					...
					char format[13];
					sprintf(format, "%%s%%s::%%0%" B_PRId32 "ld:",
						(int32)strlen(lname));	// create the sprintf format
					for (int32 j = 0; j < countfound; j++) {
						...
						ret = _StoreAttributes(&subm, f,
							lname);	 // process the sub-message with the base name
						if (ret != B_OK) {
							free(lname);
							return ret;
						}
