...
filter_result NumericFilter::Filter(BMessage* msg, BHandler** handler)
{
	...
	case B_KEY_DOWN: {
		...
		if ((modifier & B_SHIFT_KEY) == 0 && (modifier & B_CONTROL_KEY) == 0 &&
			(modifier & B_OPTION_KEY) == 0 && (modifier & B_COMMAND_KEY) == 0) {
			switch (bytes[0]) {
			case B_BACKSPACE:
			case B_LEFT_ARROW:
			case B_RIGHT_ARROW:
			...
			default:
				if (!isdigit(bytes[0])) result = B_SKIP_MESSAGE;
				break;
			}
		} else if ((modifier & B_SHIFT_KEY) != 0) {
			if (bytes[0] == B_LEFT_ARROW || bytes[0] == B_RIGHT_ARROW || bytes[0] == B_HOME ||
				bytes[0] == B_END || bytes[0] == B_TAB)
				break;
			else
