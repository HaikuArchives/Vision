...
TSpeedButton::TSpeedButton(BRect FrameRect, const char* NameStr, const char* LabelStr,
						   BMessage* InvokeMsg, BBitmap* EnabledBitmap, TSBStyle BtnStyle,
						   BBitmap* DisabledBitmap, uint32 ResizingMask, uint32 BtnFlags)
...
//-------------------------------------------------------------------------------
TSpeedButton::TSpeedButton(BMessage* Arch) : BControl(Arch)
{
	...
	if (NULL != fEnabledBitmap) SetupBitmaps(fEnabledBitmap);
...
{
	if (validate_instantiation(Archie, "TSpeedButton")) return new TSpeedButton(Archie);
...
void TSpeedButton::SetupBitmaps(BBitmap* SourceBitmap)
{
	...
	if (workRect.OffsetByCopy(EDGE_SPACE, EDGE_SPACE) != fBitmapDestinationRect) {
		...
		if (workRect.IntegerHeight() > fBitmapDestinationRect.IntegerHeight()) {
			...
			workRect.OffsetBy(
				0, (workRect.IntegerHeight() - fBitmapDestinationRect.IntegerHeight()) / 2);
		...
		if (workRect.IntegerWidth() > fBitmapDestinationRect.IntegerWidth()) {
			...
			workRect.OffsetBy((workRect.IntegerWidth() - fBitmapDestinationRect.IntegerWidth()) / 2,
							  0);
		...
		if (workRect.IntegerHeight() < fBitmapDestinationRect.IntegerHeight()) {
			...
			fBitmapDestinationRect.OffsetBy(
				0, (fBitmapDestinationRect.IntegerHeight() - workRect.IntegerHeight()) / 2);
		...
		if (workRect.IntegerWidth() < fBitmapDestinationRect.IntegerWidth()) {
			// Move the destination right by half the difference and clip
			fBitmapDestinationRect.OffsetBy(
				(fBitmapDestinationRect.IntegerWidth() - workRect.IntegerWidth()) / 2, 0);
...
void TSpeedButton::SetupLabel()
{
	...
	float availHeight = Bounds().Height() - (fontheight.ascent + fontheight.descent +
											 fontheight.leading + (2 * EDGE_SPACE));
...
void TSpeedButton::AttachedToWindow()
{
	...
	if ((NULL == fDisabledBitmap) && (NULL != fEnabledBitmap)) {
		if (fEnabledBitmap && (B_RGBA32 != fEnabledBitmap->ColorSpace())) {
			...
			if (fEnabledBitmap) fDisabledBitmap = new BBitmap(fEnabledBitmap);
		}
...
void TSpeedButton::Draw(BRect UpdateRect)
{
	...
	if (true == BControl::IsEnabled())
		if (!fOutside)
			if (fMouseDown) state = sbSDown;

	if (fStyle == sbUp)
		if (!fMouseDown || fOutside) state = sbSUp;

	if (fActive)
		if (!fMouseDown && (true == IsEnabled())) state = sbSUp;

	if (fSelected) state = sbSDown;
...
void TSpeedButton::MouseDown(BPoint Where)
{
	if (true == BControl::IsEnabled()) {
		...
		if (false == fSelected) {
			...
			if (true == fHighlighted) {
				StrokeLine(fBorder.InsetByCopy(1, 1).LeftTop(),
						   fBorder.InsetByCopy(1, 1).LeftBottom());
				StrokeLine(fBorder.InsetByCopy(1, 1).LeftTop(),
						   fBorder.InsetByCopy(1, 1).RightTop());
			}
		...
		if (fOutside == false) {
			...
			if (true == BControl::IsEnabled()) {
				if (fLatching && fSelected) {
					...
					// clean up highlight drawing.
					if (true == fHighlighted) {
						StrokeLine(fBorder.InsetByCopy(1, 1).LeftBottom(),
								   fBorder.InsetByCopy(1, 1).RightBottom());
						StrokeLine(fBorder.InsetByCopy(1, 1).RightTop(),
								   fBorder.InsetByCopy(1, 1).RightBottom());
					}
				...
				if (fGroupIndex > -1) {
					...
					while (sibView) {
						...
						if ((0 != sibling) && (this != sibling)) {
							// Only pop other buttons up if they are selected and they are of the
							// same
							// group index and they are not of the latching type.
							// Todo: determine if this should be different in terms of the latching
							// state ...
