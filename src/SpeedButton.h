...
public:
	 ...
	 *                       Enabled bitmap
	 */
	TSpeedButton(BRect Frame, const char* Name, const char* Label, BMessage* Message,
				 BBitmap* EnabledBitmap, TSBStyle Style = sbFlat, BBitmap* DisabledBitmap = NULL,
				 uint32 ResizingMask = B_FOLLOW_LEFT | B_FOLLOW_TOP, uint32 Flags = B_WILL_DRAW);
	...
	/**
	 * Set the style.
	 * @param Style Set to one of 'sbFlat' or 'sbUp'. See the constructor for explanation of the
	 * vaules.
	 */
	 ...
	 * @param Index All buttons belonging to the same index will be in the same group. A group index
	 * of
	...
	/**
	 * Primarily useful to buttons in Groups or latching, a button is seleced when it is in the
	 * 'down' state.
	 */
	...
	/**
	 * A button that 'latches' will stick down when pressed. If the button is part of a group the
	...
	/**
	 *	You may want to show that a button is in a special state, Highlighting can do this for you
