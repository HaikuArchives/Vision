	  ...
	  processing(false)
{
	...
	listMenu->AddItem(
		mFindAgain = new BMenuItem(B_TRANSLATE("Find next"), new BMessage(M_LIST_FAGAIN)));
	listMenu->AddItem(mFilter = new BMenuItem(
						  B_TRANSLATE("Filter" B_UTF8_ELLIPSIS), new BMessage(M_LIST_FILTER)));
	...
	listView = new BColumnListView("list", B_WILL_DRAW | B_NAVIGABLE | B_FULL_UPDATE_ON_RESIZE);
	...
	channelColumn = new BStringColumn(
		B_TRANSLATE("Channel"), be_plain_font->StringWidth(B_TRANSLATE("Channel")) * 2, 0, 600, 0);
	listView->AddColumn(channelColumn, 0);
	usersColumn = new BIntegerColumn(B_TRANSLATE("Users"),
		be_plain_font->StringWidth(B_TRANSLATE("Users")) * 2, 0, 200, B_ALIGN_CENTER);
	...
	switch (msg->what) {
		...
		case M_STATUS_ADDITEMS:
		{
			...
			vision_app->pClientWin()->pStatusView()->AddItem(
				new StatusItem(itemText.String(), ""), true);
		...
		case M_LIST_FILTER:
			if (msg->HasString("text")) {
				...
				PromptWindow* prompt(new PromptWindow(BPoint((Window()->Frame().right / 2) - 100,
														  (Window()->Frame().bottom / 2) - 50),
