...
#define B_TRANSLATION_CONTEXT "ParseENums"
	...
	switch (num) {
          ...
          else
          {
			...
			if (theHost != "-9z99" && theHost != "") {
				...
				thread_id lookupThread
					= spawn_thread(DNSLookup, "dns_lookup", B_LOW_PRIORITY, dnsmsg);
		...
		case RPL_ISON:	// 303
		{
			...
			for (int32 i = 0; i < fNotifyNicks.CountItems(); i++) {
				...
				if ((nickidx >= 0)
					&& ((nicks[nickidx + strlen(item->Text())] == ' ')
		...
		{
			BString theNick(GetWord(data, 4)), theIdent(GetWord(data, 5)),
				theAddress(GetWord(data, 6)), theName(RestOfString(data, 8));
			theName.RemoveFirst(":");
		...
		{
			BString theNick(GetWord(data, 4)), theIdent(GetWord(data, 5)),
				theAddress(GetWord(data, 6)), theName(RestOfString(data, 8)), tempString("[x] ");
			theName.RemoveFirst(":");
		...
		case RPL_LIST:	// 322
		{
			...
			BString channel(GetWord(data, 4)), users(GetWord(data, 5)),
		...
		{
			BString tempString(B_TRANSLATE("- Server Message Of The Day:\n"));
			Display(tempString.String(), C_SERVER, C_BACKGROUND, F_SERVER);
		}
			return true;
		...
		case ERR_RESOURCEUNAVAILABLE:  // 437
		{
			...
			if (fIsConnecting) {
				...
				if (nextNick != "") {
					BString text(
						B_TRANSLATE("* Nickname \"%nick%\" in use or unavailable... "
									"trying \"%nextnick%\"\n\n"));
					...
					Display(B_TRANSLATE("* All your pre-selected nicknames are in use.\n"));
					Display(B_TRANSLATE("* Please type /NICK <NEWNICK> to try another.\n"));
					return true;
				}
			}
			BString tempString;
			tempString
				+= B_TRANSLATE("[x] Nickname/Channel %nick% is already in use or unavailable.\n");
			tempString.ReplaceFirst("%nick%", theNick.String());
		...
		case ERR_UMODEUNKNOWNFLAG:	// 501
		{
			...
			buffer += B_TRANSLATE("[x] Unknown MODE flag.\n");
