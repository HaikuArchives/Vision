...
VTextControl::VTextControl(BRect frame, const char* name, const char* label, const char* text,
						   BMessage* vtcmessage, uint32 resizingMode, uint32 flags, bool nomenu_)
	: BTextControl(frame, name, label, text, vtcmessage, resizingMode, flags), nomenu(nomenu_)
{
	// Usage is identical to BTextControl
}

VTextControl::VTextControl(BMessage* data, bool nomenu_) : BTextControl(data), nomenu(nomenu_)
{
	// Usage is identical to BTextControl
}

void VTextControl::AllAttached(void)
{
...
void VTextControl::BuildPopUp(void)
{
	...
	bool enablecopy(true),
		...
		enablecut(true), enableselectall(true);

	int32 selstart, selfinish;
	TextView()->GetSelection(&selstart, &selfinish);
	if (selstart == selfinish) enablecopy = false; // no selection

	if (!TextView()->IsEditable() || !enablecopy) enablecut = false; // no selection or not editable
...
filter_result VTextControlFilter::Filter(BMessage* msg, BHandler**)
{
	...
	case B_MOUSE_DOWN: {
		...
		if (!parent->nomenu) {
			...
			if (mousebuttons == B_SECONDARY_MOUSE_BUTTON && (keymodifiers & B_SHIFT_KEY) == 0 &&
				...
				parent->BuildPopUp();
				parent->myPopUp->Go(parent->Parent()->ConvertToScreen(myPoint), true, false, true);
				handled = true;
			}
		}

		if (handled) result = B_SKIP_MESSAGE;
